    ...
    for cell, cell2, i in zip(self.firstColumn.cells, self.secondColumn.cells, range(0, len(self.firstColumn.cells))):
      if (type(cell2.value) != str):
        if (i != 0):
          ...
          self.d.append(
            draw.Rectangle(0, startX, self.widthView +50, height, fill=self.colorList[i], fill_opacity=0.5, stroke="white",
                           stroke_width=2))
    ...
    for cell, cell2, i in zip(self.firstColumn.cells, self.secondColumn.cells, range(0, len(self.firstColumn.cells))):
      if (type(cell2.value) != str):
        if (i != 0):
          ...
          self.d.append(
            draw.Circle(self.widthView+100, length * 80, 20, fill=self.colorList[i], fill_opacity=0.5, stroke_width=0))
