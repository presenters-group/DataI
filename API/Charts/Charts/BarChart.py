...
class BarChart (Chart):
  ...
  def drawSideLable(self):
    ...
    for i in range(0, self.quality):
      ...
      if (len(num) > 10):
        num = num[0:8] + "..."
      self.d.append(draw.Circle(self.widthOfsidelable, self.convertY(self.listofLevelValue[i]), 10,fill="lightgray", stroke_width=5, stroke='gray'))
  ...
  def drawColmunsColorList(self,fontSize:double):
    ...
    for column, i in zip(self.dataSourceTableWithoutXcolumn.columns, range(0, len(self.Xcolumn.cells))):
      ...
      if(column != self.Xcolumn):
        ...
        self.d.append(draw.Circle(add, self.heightOfbuttomLable/4+8, 15,fill=column.style.color, stroke_width=0, stroke='black'))
        self.d.append(draw.Text(text=str(num), fontSize=fontSize, x=add+25, y=self.heightOfbuttomLable / 4,
                                id=str(self.Index)))
        add += 160
  ...
  def drawColumns(self):
    for column , j in zip(self.dataSourceTableWithoutXcolumn.columns, range(0, len(self.dataSourceTableWithoutXcolumn.columns))):
      add = 250
      if (column != self.Xcolumn):
        for cell, i in zip(column.cells, range(0, len(self.Xcolumn.cells))):
          if (i != 0 ):
