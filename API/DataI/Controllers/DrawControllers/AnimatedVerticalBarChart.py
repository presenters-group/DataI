...
class AnimatedVerticalBarChart(BarChart):
    ...
    def drawColumn(self,add,colors,columnCounter,j,cell):
      ...
      if height > -1:
        t = draw.Rectangle(x, y + height, width, height, stroke=colors[columnCounter], stroke_width=self.xUnit / 60,id=self.Index,
        ...
        t = draw.Rectangle(x - self.widthOfSingleColumn, y - abs(height), width, abs(height),id=self.Index,
