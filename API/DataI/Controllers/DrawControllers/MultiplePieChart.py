...
class MultiplePieChart:
  def __init__(self, dataSource: TableModel, XColumn: ColumnModel, width: double, height: double, nameFile):
    ...
    self.total = self.sumColumn(XColumn.cells[1:])
  ...
  def sumColumn(self, column: List[CellModel]) -> double:
    sum = 0.0
    for cell in column:
  ...
  def percentageOfValue(self, value: double, column: ColumnModel) -> double:
    return double((abs(value) / self.sumColumn(column.cells[1:])) * 100)
        ...
        for cell, cell2, i in zip(self.xColumn.cells, column.cells, range(0, len(self.xColumn.cells))):
          if type(cell2.value) is float:
            if i != 0:
              ...
              p = draw.Path(stroke_width=10, stroke="white", fill=colorList[i - 1], fill_opacity=1.0, d=M + a + L)
              p.Z()
              self.d.append(p)
              if (b == 0):
                text = str(cell.value)
                self.d.append(
                  draw.Circle(self.widthView + 100, length * 80, 20, fill=colorList[i - 1], fill_opacity=0.2,
                              stroke_width=0))
