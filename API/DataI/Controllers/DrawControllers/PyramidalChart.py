        ...
        for cell, cell2, i in zip(self.firstColumn.cells, self.secondColumn.cells,
            ...
            if (type(cell2.value) != str):
                if (i != 0):
                    ...
                    self.d.append(
                        draw.Rectangle(0, startX, self.widthView - 100, height, fill=colorList[i], fill_opacity=0.5,
                                       stroke="white",
                                       stroke_width=5))
                    text = str(cell.value) + ": " + str(self.percentageOfValue(double(cell2.value)))[0:4] + "%"
                    self.d.append(
                        draw.Circle(self.widthView, length * 80, 20, fill=colorList[i], fill_opacity=0.5,
                                    stroke_width=0))
