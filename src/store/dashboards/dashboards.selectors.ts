...
} from "../visualizers/visualizers.selectors";
  ...
  (entities, visualizersEntities, filtersEntities) => {
    ...
    for (let key in entities) {
      ...
      entity.visualizers.forEach((value, key) => {
        ...
        let filters = [];
        entity.filters
          .filter((filter) => filter.visioId == value.visualizationId)
          .forEach((value, key) => {
...
export const selectVisualizersFiltersTree = createSelector(
  ...
  (visualizers, filters) => {
    ...
    for (let visualizer of Object.keys(visualizers)) {
      ...
      visualizersTree.push({
        id: visualizer,
        name: visualizers[visualizer].name,
        type: "visualizer",
        filters: filtersTree,
      });
    }
