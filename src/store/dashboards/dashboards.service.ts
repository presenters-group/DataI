...
export class DashboardsService {
  ...
  create(data: IDashboard) {
    let visualizers = data.visualizers.map((x) => ({
      visualizationId: Number.parseInt(x.visualizationId),
      measurements: x.measurements,
    }));
    let filters = data.filters.map((x) => ({
      ...x,
      id: Number.parseInt(x.id),
      visioId: Number.parseInt(x.visioId),
    }));
  ...
  updateFilterInDashboard(data) {
    return this.httpClient.put(
      `${this.URL}/update-filter/${data.dashboardId}/${data.visioId}/${data.id}/`,
      {
        id: data.id,
